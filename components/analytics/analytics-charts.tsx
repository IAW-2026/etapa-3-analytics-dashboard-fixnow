...
function JobsByCategoryChart() {
  const { data, isLoading } = useSWR<CategoryDatum[]>("jobs-by-category", fetchJobsByCategory)
      ...
      <CardContent>
        {isLoading || !data ? (
          ...
          <div className="flex flex-col items-center gap-6 sm:flex-row sm:justify-between">
            ...
            <div className="flex w-full flex-col gap-3 sm:max-w-[180px]">
              {data.map((entry) => {
...
function SuccessRateChart() {
  const { data, isLoading } = useSWR<SuccessRateDatum[]>("success-rate", fetchSuccessRate)
  ...
  return (
    <Card className="border-border">
      <CardHeader>
        ...
        <CardDescription>Completados vs. cancelados · Rider App</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading || !data ? (
