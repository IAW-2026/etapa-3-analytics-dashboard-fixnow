...
function normalizeEstadoTrabajo(status: unknown) {
  ...
  if (value === "PAID" || value === "PAGADO") return "COMPLETADO";
...
function normalizeCategoria(value: unknown): CategoriaEnum {
  ...
  if (s.includes("PLOM") || s.includes("PLUMB") || s.includes("PLOMER"))
    return "PLOMERIA";
...
export async function syncTrabajos() {
  ...
  const paymentsByJob = new Map<string, Pago>(
    (Array.isArray(paymentsData?.payments) ? paymentsData.payments : []).map(
      (p: Pago) => [String(p.jobId), p],
    ),
  );
  const reviewsByJob = new Map<string, Review>(
    (Array.isArray(feedbackData?.reviews) ? feedbackData.reviews : []).map(
      (r: Review) => [String(r.jobId), r],
    ),
  );
  ...
  for (const job of jobs) {
    ...
    const isPagado = estado === "COMPLETADO";
    const cancelacionFecha = job.cancelledAt
      ? new Date(job.cancelledAt)
      : new Date();
    const motivo = job.cancellationReason ? job.cancellationReason : "";
    const fechaFinalizacion = isPagado
      ? job.completedAt
    ...
    const jobData = {
      ...
      comisionFixNow:
        pago?.commissionAmount != null ? Number(pago.commissionAmount) : null,
